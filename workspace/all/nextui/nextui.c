	...
	while (!quit) {
		...
		if(dirty) {
			...
			if (show_version) {
				...
				if (show_setting && !GetHDMI()) GFX_blitHardwareHints(screen, show_setting);				else GFX_blitButtonGroup((const char*[]){ BTN_SLEEP==BTN_POWER?I18N_get("power"):I18N_get("menu"),I18N_get("sleep"),  NULL }, 0, screen, 0);

				GFX_blitButtonGroup((const char*[]){ I18N_get("button_b"),I18N_get("back"),  NULL }, 0, screen, 1);
			}
			else if(startgame) {
				...
				if(recents->count > 0) {
					...
					// title pill
					{
						...
						SDL_FreeSurface(text);					}					if(can_resume) GFX_blitButtonGroup((const char*[]){ I18N_get("button_b"),I18N_get("back"),  NULL }, 0, screen, 0);
					else GFX_blitButtonGroup((const char*[]){ BTN_SLEEP==BTN_POWER?I18N_get("power"):I18N_get("menu"),I18N_get("sleep"),  NULL }, 0, screen, 0);

					GFX_blitButtonGroup((const char*[]){ "Y", I18N_get("delete"), "A",I18N_get("continue"), NULL }, 1, screen, 1);
					...
					if(has_preview) {
						...
						SDL_FreeSurface(tmpsur);
						GFX_blitMessage(font.large, I18N_get("no_preview"), screen, &preview_rect);
					}
					...
					SDL_Rect preview_rect = {ox,oy,hw,hh};
					SDL_FillRect(screen, &preview_rect, 0);					GFX_blitMessage(font.large, I18N_get("no_recent_records"), screen, &preview_rect);
				...
				// buttons				if (show_setting && !GetHDMI()) GFX_blitHardwareHints(screen, show_setting);				else if (can_resume) GFX_blitButtonGroup((const char*[]){ "X",I18N_get("continue"),  NULL }, 0, screen, 0);
				...
				if (total==0) {
					...
					if (stack->count>1) {
						GFX_blitButtonGroup((const char*[]){ I18N_get("button_b"),I18N_get("back"), "A",I18N_get("open"), NULL }, 1, screen, 1);
					}
					else {
						GFX_blitButtonGroup((const char*[]){ "A",I18N_get("open"), NULL }, 0, screen, 1);
					}
				}
				...
				if (total > 0) {
					...
					GFX_blitMessage(font.large, I18N_get("empty_folder"), screen, &(SDL_Rect){0,0,screen->w,screen->h}); //, NULL);
				}
