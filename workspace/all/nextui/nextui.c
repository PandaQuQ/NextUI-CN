	...
	while (!quit) {
		...
		if(dirty) {
			...
			if (currentScreen == SCREEN_QUICKMENU) {
				...
				else GFX_blitButtonGroup((char*[]){ BTN_SLEEP==BTN_POWER?"电源":"菜单","睡眠",  NULL }, 0, screen, 0);
				...
				if(CFG_getShowQuickswitcherUI()) {
					...
					for (int c = 0; c < quickActions->count; c++) {
						...
						else if (!strcmp(item->name,"睡眠"))
							asset = ASSET_SUSPEND;
						else if (!strcmp(item->name,"重启"))
							asset = ASSET_RESTART;
						else if (!strcmp(item->name,"关机"))
							asset = ASSET_POWEROFF;
						else if (!strcmp(item->name,"设置"))
							asset = ASSET_SETTINGS;
						else if (!strcmp(item->name,"Pak 商店"))
							asset = ASSET_STORE;
